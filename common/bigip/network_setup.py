...
is_1nic = subprocess.call(
    ['/usr/bin/tmsh', 'list', 'net', 'interface', '1.0'], stdout=fnull)
if is_1nic == 0:
    onenic = True
with open('/config/network_setup.sh', 'w')as ns:
    ...
    for l in nmd['links']:
        ...
        if l['type']in ln_types:
            links[l['id']] = {'net_name': 'net_1_%s' % n_idx, 'mtu': l['mtu'], 'interface': '1.%s' % n_idx,
                              'interface_index': n_idx, 'segmentation_id': 4094-n_idx, 'tagged': False, 'route_domain': 0}
            n_idx += 1
    for l in nmd['links']:
        if l['type'] == 'vlan':
            if l['vlan_link']not in links:
                ...
                if not onenic and links[l['vlan_link']]['interface_index'] == 0:
                    ...
                    links[l['id']] = {'net_name': 'vlan_%s' % l['vlan_id'], 'mtu': links[l['vlan_link']]['mtu'], 'interface': links[l['vlan_link']]['interface'],
                                      'interface_index': links[l['vlan_link']]['interface_index'], 'segmentation_id': l['vlan_id'], 'tagged': True, 'route_domain': 0}
        ...
        if n['type']in n_types:
            ...
            for r in n['routes']:
                if n['link']in links:
                    ...
                    r['route_name'] = "route_%s" % r['network'].replace(
            ...
            if n['link']in links:
                ...
                if not n['link'] == m_l_id:
                    selfips[n['id']] = {'selfip_name': 'selfip_%s' % n['link'], 'net_name': links[n['link']]['net_name'],
                                        'ip_address': n['ip_address'], 'netmask': netmask, 'route_domain': links[n['link']]['route_domain']}
            if n['link'] == m_l_id:
                m_ip = n['ip_address']
                if 'netmask' in n:
                    m_nm = n['netmask']
